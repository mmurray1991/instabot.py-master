...
class InstaBot:
        ...
        while True:
            ...
            if len(self.media_by_tag) == 0:
                ...
                self.max_tag_like_count = random.randint(
                    1, self.max_like_for_one_tag)
            if counterlist >= len(self.media_by_tag):
                break
            # ------------------- Like -------------------
            self.new_auto_mod_like()
            # ------------------- Follow -------------------
            self.new_auto_mod_follow2(counterlist)
            # ------------------- Unfollow -------------------
    ...
    def new_auto_mod_follow2(self,count):
        if time.time() > self.next_iteration["Follow"] and \
                        self.follow_per_day != 0 and len(self.media_by_tag) > 0:
            if self.media_by_tag[0]["owner"]["id"] == self.user_id:
                self.write_log("Keep calm - It's your own profile ;)")
                return
            randomFollowInt = random.randint(0,len(self.media_by_tag))
            log_string = "Trying to follow: %s" % (
                self.media_by_tag[count]["owner"]["id"])
            self.write_log(log_string)

            if self.follow(self.media_by_tag[count]["owner"]["id"]) != False:
                self.bot_follow_list.append(
                    [self.media_by_tag[count]["owner"]["id"], time.time()])
                self.next_iteration["Follow"] = time.time() + \
                                                self.add_time(self.follow_delay)
    ...
    def unfollow_user_list(self):
        for x in self.bot_follow_list:
            if time.time() >= x[1] + 60*60*8:
                pass
